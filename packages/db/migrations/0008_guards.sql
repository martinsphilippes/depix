...
BEGIN
  ...
  -- Cada ramo precisa ser explicitamente tx_status[]: sem o cast, os ramos
  -- com literais viram text[] e o CASE não unifica com os ramos vazios.
  allowed := CASE OLD.status
    WHEN 'CREATED'         THEN ARRAY['WAITING_PAYMENT','CONVERTING','DEPIX_SENT','CANCELLED','FAILED','MANUAL_REVIEW']::tx_status[]
    WHEN 'WAITING_PAYMENT' THEN ARRAY['PIX_RECEIVED','CANCELLED','FAILED','MANUAL_REVIEW']::tx_status[]
    WHEN 'PIX_RECEIVED'    THEN ARRAY['CONVERTING','FAILED','MANUAL_REVIEW']::tx_status[]
    WHEN 'CONVERTING'      THEN ARRAY['DEPIX_SENT','FAILED','REFUNDED','MANUAL_REVIEW']::tx_status[]
    WHEN 'DEPIX_SENT'      THEN ARRAY['CONFIRMING','FAILED','MANUAL_REVIEW']::tx_status[]
    WHEN 'CONFIRMING'      THEN ARRAY['COMPLETED','FAILED','REFUNDED','MANUAL_REVIEW']::tx_status[]
    WHEN 'COMPLETED'       THEN ARRAY[]::tx_status[]
    WHEN 'FAILED'          THEN ARRAY['REFUNDED','MANUAL_REVIEW']::tx_status[]
    WHEN 'CANCELLED'       THEN ARRAY[]::tx_status[]
    WHEN 'REFUNDED'        THEN ARRAY[]::tx_status[]
    WHEN 'MANUAL_REVIEW'   THEN ARRAY['CONVERTING','DEPIX_SENT','CONFIRMING','COMPLETED','FAILED','REFUNDED','CANCELLED']::tx_status[]
  END;
